...
class Symbion_EU_Bulk_Edit {
	...
	public function add_bulk_actions( $actions ) {
		$actions['symbion_mark_as_set']     = __( 'Als Set markieren (Non-EU Restriktion)', 'symbion-eu-restriction' );
	...
	public function handle_bulk_actions( $redirect_to, $action, $post_ids ) {
		...
		foreach ( $post_ids as $post_id ) {
			...
			if ( $is_set ) {
				...
				$product->delete_meta_data( Symbion_EU_Core::META_KEY_SET );
			}
			$product->save();
			$count++;
		}
	...
	public function bulk_action_notices() {
		...
		if ( 'symbion_mark_as_set' === $action ) {
			$message = sprintf(
				...
				_n(
					'%d Produkt als Set markiert.',
					'%d Produkte als Sets markiert.',
					$count,
